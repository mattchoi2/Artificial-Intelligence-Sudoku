...
class Problem(object):
    ...
    def __init__(self, initial, goal=None, nodesGenerated=0, maxNodeMemory=0):
        ...
        self.maxNodeMemory = maxNodeMemory
...
class Node:
    ...
    def child_node(self, problem, action):
        """[Figure 3.10]"""
        # For the given action, create a new state (node)
        problem.nodesGenerated += 1
...
def breadth_first_tree_search(problem):
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            return node
        frontier.extend(node.expand(problem)) # This is where I increment # nodes generated
        # If the extended frontier is larger than the max frontier, there is a new max on the number of nodes in memory
        if (len(frontier) > problem.maxNodeMemory):
            problem.maxNodeMemory = len(frontier)
    return None
...
def idastar_search(problem, h=None):
    for depth in range(sys.maxsize):
        result = astar_depth_limited_search(problem, depth, h)
        if result != 'cutoff':
            return result

def astar_depth_limited_search(problem, limit, f):
    f = memoize(f, 'f')
    node = Node(problem.initial)
    frontier = PriorityQueue('min', f)
    frontier.append(node)
    ...
    while frontier:
        ...
        explored.add(node.state)
        for child in node.expand(problem):
            if child.depth <= limit: # This is the limit of the current depth search
                if child.state not in explored and child not in frontier:
                    frontier.append(child)
                elif child in frontier:
                    if f(child) < frontier[child]:
...
def bidirectional_search(problem):
    ...
    def extend(U, open_dir, open_other, g_dir, g_other, closed_dir):
        ...
        # Add to the number of nodes generated
...
def astar_search(problem, h=None):
    ...
    # lambdas are anonymous functions, where it is
    # "lambda <arguments, args, ...> : <expression with the args>"
    # This means we are adding the current nodes path cost (cost from curr to next)
    # PLUS the value of the heuristic function (found in GraphProblem's h())
...
class Graph:
    ...
    def __init__(self, graph_dict=None, directed=True, locations=None):
...
class GraphProblem(Problem):
    ...
    def h(self, node):
        ...
        # locations has to be a dictionary of key (name of intersection) and value (array of [longitude, latitude, elevation])
        locs = getattr(self.graph, 'locations', None)
        if locs:
            if type(node) is str:
                return float(distance(locs[node], locs[self.goal]))

            # This is my fun way to keep track of heuristic values
            self.graph.locations["heuristics"][node.state] = distance(locs[node.state], locs[self.goal])
