    ...
    def actions(self, state):
        ...
        if len(state) == len(self.variables):
            ...
            return [(var, val) for val in self.domains[var]]
                    # Commented the below out to allow for naive BFS and DFS
...
def least_constraining_variable(assignment, csp):
    # Should be the opposite of mrv
    return argmin_random_tie(
        [v for v in csp.variables if v not in assignment],
        key=lambda var: -num_legal_values(csp, var, assignment))

# Instead of a random pick, chooses the mrv variable, and on ties it will
# choose the variable with the highest degree (number of constraints with other vars)
def degree(assignment, csp):
    vars = [v for v in csp.variables if v not in assignment]
    bestNodeValue = -1
    bestNodes = []
    for var in vars:
        currNodeValue = num_legal_values(csp, var, assignment)
        if bestNodeValue == -1: # Initialize the best node value and begin the list of ties
            bestNodeValue = currNodeValue
            bestNodes.append(var)
            continue
        if currNodeValue < bestNodeValue: # Remove all ties and set new min!
            bestNodeValue = currNodeValue
            bestNodes = [var]
        elif currNodeValue == bestNodeValue: # adds to the list of ties
            bestNodes.append(var)
    ...
    # If there are any ties here, just do a random selection
    answer = argmin_random_tie(
        [v for v in bestNodes],
        key=lambda var: num_constraints(csp, var, assignment))

    return answer

def num_constraints(csp, var, assignment):
    constraintCount = 0
    for neighbor in csp.neighbors[var]: # Finds every constraint it is involved in
        if not neighbor in assignment.keys(): # Also makes sure the variable neighbor is NOT assigned yet
            constraintCount += 1
...
# should be the opposite of lcv
def most_constraining_value(var, assignment, csp):
    return sorted(csp.choices(var),
...
class NQueensCSP(CSP):
    ...
    def __init__(self, n):
        ...
        self, variables, domains, neighbors, constraints
...
class Course():
    ...
    def __init__(self, number, teacher, section, area=None):
        self.number = number
        self.teacher = teacher
        self.section = section
        self.area = area

    # This will stringify the Course
    def __repr__(self):
        return self.number + "-" + self.teacher + "-section" + str(self.section)
...
class Scheduler(CSP):
    ...
    def __init__(self, classes, slots):
        ...
        for classInfo in classes:
            number = classInfo["name"]
            areas = classInfo["areas"]
            for prof, section in classInfo["professors"].items():
                teacher = prof
                if section["num"] != "":
                    for sectionNum in list(range(int(section["num"]))):
                        currCourse = Course(number, teacher, sectionNum + 1, areas)
                        Courses.append(currCourse)
                else: # There is no "section" from the input for this class
                    Courses.append(Course(number, teacher, 1, areas))

        # for course in Courses:
        #     print(course.number + ": ")
        #     print("\t" + course.teacher)
        #     print("\tSection " + str(course.section))
        #     print("\t" + str(course.area))
        # Pack them all in a dictionary
        neighbors = {}

        for course in Courses:
            # IDK why, but we need to stringify all the classes to a UNIQUE string for the KEYS
            # The VALUES in this case will be a range of time slots, from 0 to n
            self.domains[str(course)] = list(range(int(slots)))
            # print(str(course))

        # USEFUL INFO ==========
        # The courses are our variables (an array of classes)
        # The domain is all available time slots.  Each class (variable) is assigned a time slot (domain)
        # classes into multiple strings
        # two classes are neighbors if they share a course code, professor, or area
        # neighbors - dictionary (key is each individual sections into string, values are sets(python data type)
            # key is classNUmber-professor-sectionNumber
            # value set containing the same thing as key, all things that share a course code, professor, or area
        # ======================

        for course in Courses:
            neighborSet = set() # Uses python SET data structure
            # print(str(course))
            for neighbor in Courses:
                if str(neighbor) == str(course):
                    continue # you can't be a neighbor of yourself
                if neighbor.teacher == course.teacher or neighbor.number == course.number:
                    neighborSet.add(str(neighbor))
                # Neighbors also if BOTH the neighbor and couse have an area, and the area is the same
                if neighbor.area != None and course.area != None:
                    for area in neighbor.area:
                        if area in course.area:
                            neighborSet.add(str(neighbor))

                    # print("\t" + str(neighbor))
            # Assign the course variable to all its course neighbors in the set
            neighbors[str(course)] = neighborSet

        CSP.__init__(self, None, self.domains, neighbors, different_values_constraint)
...
class Sudoku(CSP):
    ...
    bgrid = _BGRID
    boxes = _BOXES
    rows = _ROWS # The single _var notation means the variable is used internally and privately
    cols = _COLS
    ...
    def __init__(self, grid):
        ...
        # iter() creates an iterable object over the data passed in
        # uses regular expression library (re) to find all the regex matches in the parameter passed in
        # squares is used to create the domain?
            # \d is digits [0-9]
            # | means OR
            # \. is an escaped search for the "." character
        squares = iter(re.findall(r'\d|\.', grid))
        domain = '123456789'
        if (len(grid) == 16): # could have a 4x4 where only these values are domain
            domain = '1234'
        # Code within {} signifies a dictionary (key->value pair)
        # flatten() will convert a 2D array to a 1D array
        # zip() will return a list containing tuples of the rows of the board => a list of digits 0-9 and "."s
        domains = {var: [ch] if ch in domain else domain
                   for var, ch in zip(flatten(self.rows), squares)}
        # a convention in python where _ is the iteration variable that's not used
        for _ in squares:
